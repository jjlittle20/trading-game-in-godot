...
func load_player_state():
	...
	if loaded_data == null:
		push_error("SaveService could not load player data from: %s" % path)
		return null

	if not loaded_data is Dictionary:
		push_error("Player save root must be a Dictionary: %s" % path)
		return null
...
func save_player_state(player_state) -> bool:
	...
	if not saved:
		push_error("SaveService could not write player data to: %s" % user_save_path)
		return false
